...
from all_namespace import namespace, create_move_wizard, cast_spell, create_select_spell, create_toggle_ai, \
    create_get_wizard_health, create_is_wizard_visible, create_get_distance
...
class Wizard:
    def __init__(self, x: int, y: int, name: str):
        ...
        self.score = 0
        self.vision_range = 4  # Радиус видимости в клетках
        self.original_vision_range = 4
        self.searching = True  # Initial state: searching for the opponent
        ...
        for dy in range(-self.vision_range, self.vision_range + 1):
            for dx in range(-self.vision_range, self.vision_range + 1):
                ...
                if game_map.is_valid_position(check_x, check_y):
                    # Проверяем расстояние до клетки (используем круговую видимость)
...
class WizardGame:
    ...
    def handle_input(self):
        ...
        if keys[pygame.K_LEFT]:
            self.move_wizard(self.current_wizard.name, -1, 0)
        if keys[pygame.K_RIGHT]:
            self.move_wizard(self.current_wizard.name, 1, 0)
        if keys[pygame.K_UP]:
            self.move_wizard(self.current_wizard.name, 0, -1)
        if keys[pygame.K_DOWN]:
            self.move_wizard(self.current_wizard.name, 0, 1)

        # Switch between wizards with Tab
        if keys[pygame.K_TAB]:
            if self.current_wizard == self.gandalf:
                self.current_wizard = self.merlin
            else:
                self.current_wizard = self.gandalf
    ...
    def execute_strategy(self, wizard_name):
        ...
        if commands:
            command = commands[0]  # Берем только первую команду за раз
            self.active_console = self.gandalf_console if wizard_name == "Гэндальф" else self.merlin_console
            self.active_console["input"] = command
    ...
    def ai_move(self):
        ...
        merlin = self.merlin
        gandalf = self.gandalf

        self.regenerate_mana(merlin, 5)

        # Если Мерлин ищет противника
        if merlin.searching:
            # Случайное перемещение для поиска
            dx, dy = random.choice([(0, 1), (1, 0), (0, -1), (-1, 0)])
            new_x, new_y = merlin.x + dx, merlin.y + dy
            if self.game_map.is_valid_position(new_x, new_y):
                merlin.x, merlin.y = new_x, new_y
                merlin.direction = (dx, dy)

            # Проверка, видит ли Мерлин Гэндальфа
            visible_cells = merlin.get_visible_cells(self.game_map)
            if (gandalf.x, gandalf.y) in visible_cells:
                merlin.searching = False  # Прекращаем поиск, начинаем атаковать
            return

        # Если Мерлин видит Гэндальфа, атакуем
        visible_cells = merlin.get_visible_cells(self.game_map)
        if (gandalf.x, gandalf.y) in visible_cells:
            dx = gandalf.x - merlin.x
            dy = gandalf.y - merlin.y

            # На одной ли линии волшебники
            if (dx == 0 or dy == 0 or abs(dx) == abs(dy)) and merlin.mana >= 20:
                if dx != 0:
                    dx = dx // abs(dx)
                if dy != 0:
                    dy = dy // abs(dy)

                x, y = merlin.x, merlin.y
                path_clear = True

                while (x != gandalf.x or y != gandalf.y) and path_clear:
                    x += dx
                    y += dy
                    if self.game_map.is_wall(x, y) and (x != gandalf.x or y != gandalf.y):
                        path_clear = False

                if path_clear:
                    merlin.direction = (dx, dy)
                    # Выбор заклинания в зависимости от расстояния
                    distance = max(abs(gandalf.x - merlin.x), abs(gandalf.y - merlin.y))

                    if distance <= 3:
                        merlin.selected_spell = "ледяной осколок"
                    elif distance <= 5:
                        merlin.selected_spell = "огненный шар"
                    else:
                        merlin.selected_spell = "молния"

                    # Есть ли мана
                    spell = self.spells[merlin.selected_spell]
                    if merlin.mana >= spell.mana_cost:
                        merlin.mana -= spell.mana_cost
                        self.projectiles.append(SpellProjectile(
                            merlin.x, merlin.y,
                            dx, dy,
                            spell, merlin
            ...
            # Если Гэндальф не виден, возобновляем поиск
            merlin.searching = True
        ...
        for move_dx, move_dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            new_x, new_y = merlin.x + move_dx, merlin.y + move_dy

            if self.game_map.is_valid_position(new_x, new_y):
                # Рассчитаем расстояние до Гэндальфа после перемещения
                distance = abs(new_x - gandalf.x) + abs(new_y - gandalf.y)
        ...
        if possible_moves:
            ...
            merlin.x += move_dx
            merlin.y += move_dy
            merlin.direction = (move_dx, move_dy)
        ...
        while self.running:
            ...
            if not self.game_over:
                if not self.game_started:
                    ...
                    self.handle_input()
                    ai_timer += 1
                    if self.ai_enabled and ai_timer >= 10:  # Увеличиваем частоту обновления
                        ai_timer = 0
